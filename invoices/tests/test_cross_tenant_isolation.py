"""Cross-tenant isolation contract for invoices."""
...
def _restore_rls_state() -> Generator[None, None, None]:
    """Make sure RLS is off for the invoices tables around this test.
    ...
    manually re-enables RLS on ``invoices_invoice`` between its data
...
def test_rls_blocks_cross_tenant_invoices():
    """Setting a session tenant filters Invoice rows to that tenant."""
    ...
    ca = Client.objects.create(tenant=ta, name="A Co", email="a@a.io")
    cb = Client.objects.create(tenant=tb, name="B Co", email="b@b.io")
    Invoice.objects.create(
        tenant=ta, client=ca, number="INV-A", due_date=date(2026, 12, 31)
    )
    Invoice.objects.create(
        tenant=tb, client=cb, number="INV-B", due_date=date(2026, 12, 31)
    )

    # Re-enable RLS so the policy actually applies to the SELECT.
    enable_rls_on("invoices_invoice")
    try:
        set_session_tenant(ta.id)
        with connection.cursor() as cur:
            cur.execute("SELECT tenant_id FROM invoices_invoice")
            rows = [r[0] for r in cur.fetchall()]
        # The RLS contract: no rows from the other tenant are visible.
        assert rows, "expected at least one invoice row for tenant A"
